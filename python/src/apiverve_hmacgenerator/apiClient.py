...
class HmacAPIClientError(Exception):
...
class ValidationError(HmacAPIClientError):
...
class HmacAPIClient:
    ...
    VALIDATION_RULES = {"message": {"type": "string", "required": True}, "secret": {"type": "string", "required": True}, "algorithm": {"type": "string", "required": False, "default": "sha256"}, "encoding": {"type": "string", "required": False, "default": "hex"}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the HmacAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises HmacAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/hmac'
        self.headers = {
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[HmacAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns HMACGeneratorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises HmacAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise HmacAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise HmacAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise HmacAPIClientError(f"Request failed: {str(e)}")
