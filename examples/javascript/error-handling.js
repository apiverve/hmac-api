/**
 * HMAC Generator API - Error Handling Example
 *
 * This example demonstrates proper error handling when using the HMAC Generator API.
 ...
 */
async function callHMACGeneratorAPIWithErrorHandling(queryParams = {}) {
  try {
...
async function callWithRetry(maxRetries = 3, initialDelay = 1000) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const result = await callHMACGeneratorAPIWithErrorHandling({
